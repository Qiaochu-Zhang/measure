...
适用版本：`V1_18`（2026-09-22）。主程序：`sem_cd_measure_200k_batch_V1_18.py`，基于 V1_17（2301b35），将 trench 改为先测全部合规候选、再筛选合格结果，保留 line 共享边和自动 pitch 测量，按本次 V1_18 发布的实际参数解析、校验和执行代码编写。本文覆盖主程序全部 **86 个参数选项（包括帮助选项，`-h` 和 `--help` 为同一个选项）**，另列自检脚本参数。没有在命令行开放的内部常量，不应当作可传入参数。

- [完整代码 ZIP](sem_cd_measure_200k_batch_V1_18_complete_package.zip)
- [版本概述](V1_18_RELEASE_NOTES.md)
- [包内 README](sem_cd_measure_200k_batch_V1_18_complete_package/README.md)
- [主程序源码](sem_cd_measure_200k_batch_V1_18_complete_package/sem_cd_measure_200k_batch_V1_18.py)

## 1. 先了解默认行为

程序用于 PNG 图像中近竖直的暗沟槽 `trench` 或亮线 `line` 的 CD、LER、LWR 测量。一次运行的整个输入目录只能使用一种图案类型；不同类型请分开运行。本版不支持 via，也不自动逐图判断 trench/line。V1_18 可单独解压运行，输出版本标识、Excel 及默认目录均为 V1_18；递归扫描仍排除 V1_14/V1_15/V1_16/V1_17/V1_18 的历史结果。建议不同版本结果放在输入目录之外。
...
解压完整 ZIP，进入包含主程序和 `requirements_V1_18.txt` 的目录。建议 Python 3.12；代码使用 Python 3.10+ 语法，实际测试版本见包内 `validation_report.json`。
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root examples/input_trench --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 4 --output demo_output --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root "D:/SEM/input_trench" --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output "D:/SEM/results_trench_01" --no-auto-machine-comparison
```

亮线：

```bash
python sem_cd_measure_200k_batch_V1_18.py --root "D:/SEM/input_line" --pattern line --pixel-size 1.3181 --line-reference-nm 60 --output "D:/SEM/results_line_01" --no-auto-machine-comparison
```

Linux/macOS 把路径替换为 `/path/to/images` 等实际路径。相对路径基于终端的当前工作目录；含空格的路径加引号。Windows 也可使用 `./RUN_V1_18.ps1`，Linux/macOS 可用 `./run_V1_18.sh` 替代 `python sem_cd_measure_200k_batch_V1_18.py`，参数相同。PowerShell 包装脚本未在本次 Linux 环境执行验证。
...
| `--output` | 路径；自动 | 默认 `root/CD_measure_output_200K_V1_18`。目录必须不存在或为空。不要直接指定到包含输入图的目录。 |
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output results_basic --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --no-auto-roi --center-x 300 --center-y 250 --meas-area-width 400 --meas-area-height 300 --extend-length 240 --output results_manual --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern line --pixel-size 1.3181 --line-reference-nm 60 --max-number 1 --min-number 1 --group-size 1 --output results_single --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source single-row --psd-gap-mode interpolate --psd-max-gap 2 --output results_psd_interpolate --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source measurement --output results_psd_reuse --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --viterbi 1 --viterbi-weight 5 --viterbi-max-jump 3 --erf-fit 1 --erf-window 8 --save-debug-masks --output results_path_erf --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --skip-psd --skip-statistics-plots --no-annotated-images --compact-sample-output --output results_fast --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --pattern trench --max-number 4 --group-size 8 --check-parameters
```
...
先打开 `CD_measurement_200K_V1_18_results.xlsx` 的 `measurement_summary`，再核查 `ROI/` 和双引擎标注图。
...
| `machine_comparison_detailed.csv`、`V1_18_statistics_and_machine_comparison.xlsx` | 有参考表时的逐图机台对比和统计摘要；另查 unmatched 表定位匹配问题。 |
...
自检脚本 `self_check_V1_18.py` 不是主测量入口，它的参数为：

| 参数 | 默认值 | 含义 |
|---|---|---|
| `-h` / `--help` | 不启用 | 查看自检脚本帮助。 |
| `--trench-only` | 仅运行解析检查及 V1_18 trench 全测后筛选专项回归；不能代替完整发布自检。 |
...
python sem_cd_measure_200k_batch_V1_18.py --help
```

随包报告包含原有端到端/故障场景，以及新增模式判断、两种定位、端部与 PSD 排除、参数检查；这些是软件回归与随包/合成图验证，不替代真实 SEM 标准样验证。

本手册的覆盖范围以主程序 `build_parser()` 的全部选项为准：编写后逐项检查参数表是否包含每个公开参数，并使用实际解析器检查示例命令及相互冲突的参数。内部 `via_reference_nm` 等历史兼容字段并不是公开接口。本版使用独立 V1_18 包；原 V1_15、V1_16、V1_17 的代码、文档、测试报告和 ZIP 与修改前提交 2301b35 保持字节一致。包内附有同内容的 RUN_GUIDE.md，离线解压即可查阅全部参数。
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --locator-mode auto --pixel-size 1.3181 --trench-reference-nm 60 --output result_auto --no-auto-machine-comparison
```

已知 line 内有假暗条：

```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --locator-mode dark-line --pixel-size 1.3181 --trench-reference-nm 60 --output result_dark_line --no-auto-machine-comparison
```

已知 line 整体亮：

```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --locator-mode bright-line --locator-majority 0.7 --end-trim-fraction 0.05 --pixel-size 1.3181 --trench-reference-nm 60 --output result_bright_line --no-auto-machine-comparison
```

对照 V1_14 关闭自动 ROI 时的边缘配方（保留 bounded 搜索，最终 trench 筛选按 V1_18 执行）：

```bash
python sem_cd_measure_200k_batch_V1_18.py --root input --pattern trench --locator-mode dark-line --no-auto-roi --end-trim-fraction 0 --pixel-size 1.3181 --trench-reference-nm 60 --output result_v114_compatible --no-auto-machine-comparison
```

若复现的是 V1_14 的 threshold-search=legacy 配方，还需同步加上 `--threshold-search legacy`。其余标定、ROI、采样、参考宽度和统计参数也须一致。V1_18 排除不合格 trench，参与汇总的对象可能变化，因此不承诺复现旧版纳入不合格对象的汇总值；同一来源 trench 的逐点算法保持不变。开启新区域裁剪会改变参与测量的像素/长度，因此不能要求结果与未裁剪版本数值一致。
...
```bash
python sem_cd_measure_200k_batch_V1_18.py --root examples/input_dark_line --pattern line --pixel-size 1 --line-reference-nm 100 --space-reference-nm 60 --max-number 3 --output result_line_pitch --no-auto-machine-comparison
```

例：同图测 trench 并自动测相邻周期：

```bash
python sem_cd_measure_200k_batch_V1_18.py --root examples/input_dark_line --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 3 --output result_trench_pitch --no-auto-machine-comparison
```

完整自检仍用 `python self_check_V1_18.py --output validation_report.json`；line/pitch 测试在 `check_line_pitch.py`，共享边缘测试在 `check_shared_lines.py`。覆盖逐点共享边验证、宽暗条、自动估宽、Viterbi/ERF、左右不同阈值、全部 trench 搜索、max-number=1/2/5、只有一条/两条 trench、不合格及缺失中间 trench、独立 PSD、倾斜周期、背景/合成点排除和 Excel 末列验证。随包样例及新增图均为合成图，尚无用户实测失败 SEM 图用于验证。


## 13. V1_18：trench 先全测、再筛选（2026-09-22）

V1_17 的 trench 在逐点测量前先选定几条候选，可能保留断续的沟槽，默认旧中心选择还可能选到假暗条；line 则先测全部来源 trench，只用相邻且合格的来源构成 line。V1_18 将 trench 的测量/筛选顺序调整为后者，最终输出仍是 trench 本身。
...
python self_check_V1_18.py --trench-only --output trench_validation.json
python self_check_V1_18.py --output validation_report.json
```
