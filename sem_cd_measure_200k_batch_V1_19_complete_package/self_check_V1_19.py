...
def analytical_tests():
    ...
    require(
        app.SCRIPT_VERSION == "V1_19" and app.PATCH_VERSION == "V1_19",
        "independent V1_19 version",
    )
    with tempfile.TemporaryDirectory(prefix="v119-version-check-") as directory:
        root = Path(directory)
        gray = app.regions.read_gray_png(BASE / "examples/input_trench/five/test.png")
        app.regions.unicode_imwrite(root / "input.png", gray)
        for version in ("V1_14", "V1_15", "V1_16", "V1_17", "V1_18", "V1_19"):
        ...
        require(
            config.resolved_output_dir().name == "CD_measure_output_200K_V1_19",
            "V1_19 default output",
        )
    ...
    report["v119_version_isolation"] = {
        "status": "PASS",
        "excluded_output_versions": ["V1_14", "V1_15", "V1_16", "V1_17", "V1_18", "V1_19"],
    }
    ...
    from check_shared_lines import analytical_tests as shared_tests

    report["shared_lines"] = shared_tests(require, app)
    from check_trench_selection import analytical_tests as trench_tests

    report["trench_selection"] = trench_tests(require, app)
    from check_image_formats import analytical_tests as format_tests

    report["image_formats"] = format_tests(require, app)
    return report


def integration_tests(temp, trench_only=False, formats_only=False):
    cases, cached = {}, {}

    def execute(name, root=None, extra=(), pattern="trench", reference=60, plots=False):
        output = temp / name
        ...
        args = [
            ...
            *(
            ...
            "--max-number",
            "3" if pattern == "line" else "4",
            "--no-auto-machine-comparison",
            *([] if plots else ["--skip-statistics-plots"]),
            *extra,
        ]
        ...
        book = load_workbook(
            output / "CD_measurement_200K_V1_19_results.xlsx", read_only=True
        )
        require(
    ...
    from check_trench_selection import integration_tests as trench_tests
    from check_image_formats import integration_tests as format_tests

    if formats_only:
        cases["image_formats"] = format_tests(temp, execute, require, app)
        return cases

    if trench_only:
        cases["trench_selection"] = trench_tests(temp, execute, require, app)
        return cases
    ...
    require(line.status.eq("OK").all(), "shared line default status")
    verify_shared_exports(output, require)

    legacy_output, legacy = execute(
        "legacy_reproduction",
        extra=[
            "--skip-psd",
            "--no-annotated-images",
    ...
    from check_trench_selection import verify_legacy_quality_gate

    verify_legacy_quality_gate(legacy_output, legacy, require)
    _, single = execute(
        "one_group",
    ...
    require((output / "ROI/offset.png.png").is_file(), "ROI diagnostic overlay")
    ...
    from check_shared_lines import integration_tests as shared_tests

    cases["shared_lines"] = shared_tests(temp, execute, require, app)
    cases["trench_selection"] = trench_tests(temp, execute, require, app)
    cases["image_formats"] = format_tests(temp, execute, require, app)
    return cases
...
if __name__ == "__main__":
    ...
    scope.add_argument(
        "--trench-only",
        action="store_true",
        help="Run analytical and V1_19 trench selection tests only",
    )
    scope.add_argument(
        "--formats-only",
        action="store_true",
        help="Run analytical and V1_19 image format integration tests only",
    )
    ...
    if not args.quick:
        with tempfile.TemporaryDirectory(prefix="measure-v119-check-") as temp:
            result["integration"] = integration_tests(
                Path(temp), trench_only=args.trench_only, formats_only=args.formats_only
            )
    if args.output:
    ...
    print("V1_19 SELF-CHECK PASSED", flush=True)
