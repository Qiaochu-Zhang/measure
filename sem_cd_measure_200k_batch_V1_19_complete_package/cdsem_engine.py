...
def detect_space_candidates(
    ...
    *,
    ...
    chooser_params = params
    if all_candidates:
        from dataclasses import replace

        chooser_params = replace(
            params,
            max_number=image.shape[1],
            min_number=1,
...
def measure_image(
    ...
    if params.pattern_kind == "line":
        from cdsem_lines import measure_lines

        return measure_lines(path, params, image, load_warnings, region_mask)
    from cdsem_trenches import measure_trenches

    return measure_trenches(path, params, image, load_warnings, region_mask)
