# SEM CD / LER / LWR — V1_19

当前版本 `V1_19`（2026-09-23）：支持混合格式图片输入，包括 TIF/TIFF、PNG、JPG/JPEG、BMP、WebP，扩展名不区分大小写。保留 V1_18 的 trench 全量测量后筛选、line 共享边和 pitch 测量算法。

基于 V1_18（1fcde1c）的独立完整包，不依赖旧目录。旧版本代码、文档、报告和 ZIP 保持原样。详见 [修改记录](CHANGELOG.md) 与 [图片格式说明](RUN_GUIDE.md#14-v1_19多格式图片输入2026-09-23)。
...
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_bright_line --pattern trench --pixel-size 1 --trench-reference-nm 60 --output demo_output --no-auto-machine-comparison
```

自己的图片：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root /path/to/images --pattern trench --locator-mode auto --pixel-size 1.3181 --trench-reference-nm 60 --end-trim-fraction 0.05 --output /path/to/new_results
```

像素标定和参考宽度必须按实际图片替换。输出目录须不存在或为空。Linux/macOS 可用 `./run_V1_19.sh`，Windows 可用 `./RUN_V1_19.ps1`，参数原样转发；PowerShell 包装器未在本次 Linux 环境执行。

## 图片格式

输入仍通过 `--root` 指定目录，自动递归读取 `.tif`、`.tiff`、`.png`、`.jpg`、`.jpeg`、`.bmp`、`.webp`，可混合放置，支持中文路径和大写扩展名。无需增加参数或依赖。支持 8/16 位灰度 TIFF/PNG 和彩色图；彩色图转灰度，非 8 位图按原规则归一化。多页 TIFF 仅读取第一页，不展开为多张图，也不从 TIFF 元数据自动推断像素标定。JPEG 有损压缩可能改变边缘和粗糙度，测量时优先使用原始无损图。

ROI、标注图、debug、PSD 图统一导出 PNG，并保留完整输入文件名，例如 `sample.tif.png`、`sample.jpg.psd.png`、`sample.png_V10_rotated.png`，避免同名不同格式互相覆盖。CSV/Excel 的 image/image_key 保留原始扩展名和相对路径。

`settings.json` 使用 `input_image_count` / `processed_image_count`，并记录 `supported_image_suffixes`；机台未匹配图片表改名为 `machine_unmatched_images.csv`。机台比较仍以去扩展名的文件名匹配，同名不同格式会作为歧义排除，并写明原因。损坏的支持格式文件记录 ERROR，默认继续处理其他图片。
...
- `CD_measurement_200K_V1_19_results.xlsx`：首表 `measurement_summary` 每图 mixed、V13、V10 三行；最后一列 `旋转_pitch_CD_nm`。
...
新增真实编码文件的格式解码与混合批处理回归，覆盖大小写、中文路径、16 位灰度、彩色/透明图、多页 TIFF 第一页、同名导出防覆盖、损坏文件及机台匹配。保留 trench 全测后筛选回归，覆盖假中心、max-number=1/2/5、合格数量不足/为零、单 trench、line/trench 同条件来源逐点一致和独立 PSD 筛选。测试继续涵盖原有边缘数值回归、背景、默认双引擎/双极性、Viterbi/ERF、PSD、故障导出和防覆盖，并新增两种定位、auto/手动一致性、噪声/灰度偏移、窄假暗条、不同长度端部及强制恢复/PSD 插值排除测试。实际结果见 [validation_report.json](validation_report.json)。
...
| 文件 | 职责 |
|---|---|
| `sem_cd_measure_200k_batch_V1_19.py` | 参数、区域/模式编排、双引擎、结果导出 |
...
| `cdsem_regions.py` / `check_image_formats.py` | 多格式读图、输出命名及格式回归；背景排除、端部裁剪、区域图 |
| `cdsem_engine.py` | 两种定位共用的边缘引擎 |
| `cdsem_refinement.py` / `cdsem_statistics.py` / `cdsem_psd.py` | 延用的修正、统计和 PSD |
| `self_check_V1_19.py` / `check_localization.py` | 完整自检及新算法测试 |

内部部分函数名包含 v115 或 V1.15，是继承的历史算法标识；当前发布版本以主程序 SCRIPT_VERSION=V1_19 为准。PACKAGE_SHA256.txt 校验全部交付文件，不包括清单自身。

## 测 line

```bash
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_dark_line --pattern line --pixel-size 1 --line-reference-nm 100 --space-reference-nm 60 --max-number 3 --output results_line_V1_19 --no-auto-machine-comparison
```
