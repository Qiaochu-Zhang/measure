"""V1_19: consolidated active implementation, derived from V13_modified."""
...
class GeneralConfig:
    ...
    def resolved_output_dir(self) -> Path:
        ...
        return self.root_dir / "CD_measure_output_200K_V1_19"
...
def read_gray_image(path: Path) -> np.ndarray:
    """Decode a supported image (first TIFF page) using the existing gray scale."""
    return normalize_gray_uint8(unicode_imread(path))


# Compatibility alias for scripts importing the V1_18 reader.
read_gray_png = read_gray_image


def output_image_path(key: str, suffix: str = ".png") -> Path:
    """Keep the complete input name so different formats never share an export."""
    path = Path(key)
...
def is_supported_image(path: Path) -> bool:
    ...
    return path.is_file() and path.suffix.casefold() in SUPPORTED_IMAGE_SUFFIXES
...
def parse_image_from_content(
    ...
    trench_nm, space_nm = estimate_trench_references_nm(gray, config.pixel_size_nm)
    if pattern == "line":
        from cdsem_localization import analyze_mode

        mode = analyze_mode(gray)
        period = mode.get("locator_adjacent_period_px")
        line_nm = (
            period * config.pixel_size_nm - trench_nm
            if period is not None
            else space_nm
        )
        trench_nm, space_nm = (line_nm if line_nm > 0 else space_nm), trench_nm
...
def discover_images(
    ...
    paths = sorted(
        (p for p in root.rglob("*") if is_supported_image(p)),
