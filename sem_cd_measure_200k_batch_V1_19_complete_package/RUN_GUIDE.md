...
适用版本：`V1_19`（2026-09-23）。主程序：`sem_cd_measure_200k_batch_V1_19.py`，基于 V1_18（1fcde1c），新增多格式图片输入，保留 trench 全测后筛选、line 共享边和自动 pitch 测量，按本次 V1_19 发布的实际参数解析、校验和执行代码编写。本文覆盖主程序全部 **86 个参数选项（包括帮助选项，`-h` 和 `--help` 为同一个选项）**，另列自检脚本参数。没有在命令行开放的内部常量，不应当作可传入参数。

- [完整代码 ZIP](https://github.com/Qiaochu-Zhang/measure/blob/main/sem_cd_measure_200k_batch_V1_19_complete_package.zip)
- [版本概述](CHANGELOG.md)
- [包内 README](README.md)
- [主程序源码](sem_cd_measure_200k_batch_V1_19.py)

## 1. 先了解默认行为

程序用于 TIF/TIFF、PNG、JPG/JPEG、BMP、WebP 图像中近竖直的暗沟槽 `trench` 或亮线 `line` 的 CD、LER、LWR 测量。一次运行的整个输入目录只能使用一种图案类型；不同类型请分开运行。本版不支持 via，也不自动逐图判断 trench/line。V1_19 可单独解压运行，输出版本标识、Excel 及默认目录均为 V1_19；递归扫描仍排除 V1_14/V1_15/V1_16/V1_17/V1_18/V1_19 的历史结果。建议不同版本结果放在输入目录之外。

默认会：

1. 递归读取输入目录及子目录的 TIF/TIFF、PNG、JPG/JPEG、BMP、WebP 图片（扩展名大小写不限，可混合格式），不按文件名或 `200K` 字样筛选；当前输出目录和可识别的历史输出子目录会被排除。
2. 筛除背景，按估计平均 trench 长度从 Y 两端各去掉 5%，自动确定未手动指定的 ROI；默认每图最多选 3 条结构；trench 和 line 均先测 ROI 内全部合规 trench；trench 筛选合格沟槽输出，line 取相邻合格沟槽之间的间隙。max-number 在测完并筛选后限制最终条数。根据整个有效区域的 X 灰度周期逐图选择 dark-line 或 bright-line 定位器。
...
解压完整 ZIP，进入包含主程序和 `requirements_V1_19.txt` 的目录。建议 Python 3.12；代码使用 Python 3.10+ 语法，实际测试版本见包内 `validation_report.json`。
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_trench --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 4 --output demo_output --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root "D:/SEM/input_trench" --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output "D:/SEM/results_trench_01" --no-auto-machine-comparison
```

亮线：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root "D:/SEM/input_line" --pattern line --pixel-size 1.3181 --line-reference-nm 60 --output "D:/SEM/results_line_01" --no-auto-machine-comparison
```

Linux/macOS 把路径替换为 `/path/to/images` 等实际路径。相对路径基于终端的当前工作目录；含空格的路径加引号。Windows 也可使用 `./RUN_V1_19.ps1`，Linux/macOS 可用 `./run_V1_19.sh` 替代 `python sem_cd_measure_200k_batch_V1_19.py`，参数相同。PowerShell 包装脚本未在本次 Linux 环境执行验证。
...
| `--root` | 路径；`.` | 输入根目录，递归扫描支持的图片格式（见第 14 节）；必须是存在的文件夹，不是单张图片路径。 |
| `--output` | 路径；自动 | 默认 `root/CD_measure_output_200K_V1_19`。目录必须不存在或为空。不要直接指定到包含输入图的目录。 |
...
| `--space-reference-nm` | 浮点数；不显式约束 | 目标之间的间隙宽度，必须 >0，**不是中心距**。trench 模式为 line 宽度，显式设置时用于邻居排序先验；line 模式为物理 trench 宽度，未设置时从原图估计，用于搜索和测量共享边缘的来源 trench。 |
| `--max-number` | 整数；`3` | 每张图每个引擎最多选取的结构数量，≥1；同时为 pitch 求平均的目标周期数。N 个完整 pitch 需要 N+1 条物理 trench 的边缘；trench 和 line 均先测全部合规 trench，来源搜索不提前受此数量截断。不是图片数量，也不是采样点数。 |
| `--min-number` | 整数；`min(3,max-number)` | 质量判定要求的最少稳定结构数，≥1 且不能超过 max-number。trench 数量不足时仅保留已合格结果，不用不合格沟槽补数，通常标记 REVIEW；全无合格结果则该引擎无测量值。line 保留 V1_17 的质量规则。 |
| `--allow-incomplete-triplet` | 开关；默认不启用 | 取消“中心结构及左右邻居齐全”的额外质量要求。不会取消 min-number 要求，也不会凭空增加候选；max-number <3 时原本就不要求完整三条。 |
| `--candidate-dark-tolerance` | 浮点数；`None` | 可选的候选核心灰度容差，≥0：以中心候选核心灰度为参考，过滤比其更亮且超出容差的候选。line 和 trench 均作用于原图中的暗 trench 候选。不是左右边缘阈值。 |
| `--candidate-min-contrast` | 浮点数；`None` | 可选的来源 trench 候选最低对比度，≥0；过滤粗定位候选。未指定表示不额外施加此门槛，并不关闭定位器原有检查。 |
...
V10 通常使用中心暗参考及峰附近参考，V13 使用侧带均值等参考；当中心受内部条带污染、两侧内部平台一致且更暗时，改用两侧内部平台的中位数均值作为暗参考，这是继承的 trench 阈值规则。即使同为 50%，交点也不一定相同。line 模式直接使用相邻两条 trench 的内侧边缘。threshold-left/right 作用于来源 trench：line 左边继承左 trench 的 right 阈值，line 右边继承右 trench 的 left 阈值；两者不同时尤其需要注意。
...
对比使用主程序的 mixed 指标（旋转、未旋转分别比较），默认只纳入 OK。匹配键基于文件名去扩展名、忽略大小写，并去掉末尾类似 `(1)` 的数字后缀；不是按完整相对路径匹配。不同子目录或不同格式存在同名图片可能被判为歧义并排除匹配，应检查 `machine_unmatched_images.csv`。未匹配参考行见 `machine_unmatched_rows.csv`。
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output results_basic --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --no-auto-roi --center-x 300 --center-y 250 --meas-area-width 400 --meas-area-height 300 --extend-length 240 --output results_manual --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern line --pixel-size 1.3181 --line-reference-nm 60 --max-number 1 --min-number 1 --group-size 1 --output results_single --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source single-row --psd-gap-mode interpolate --psd-max-gap 2 --output results_psd_interpolate --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source measurement --output results_psd_reuse --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --viterbi 1 --viterbi-weight 5 --viterbi-max-jump 3 --erf-fit 1 --erf-window 8 --save-debug-masks --output results_path_erf --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --skip-psd --skip-statistics-plots --no-annotated-images --compact-sample-output --output results_fast --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --pattern trench --max-number 4 --group-size 8 --check-parameters
```
...
先打开 `CD_measurement_200K_V1_19_results.xlsx` 的 `measurement_summary`，再核查 `ROI/` 和双引擎标注图。
...
| `trench_selection.csv` / 同名工作表 | trench 全部已测候选的 ID、有效比例、合格性、最终入选状态和原因；SELECTED / QUALIFIED_NOT_SELECTED / UNQUALIFIED。 |
| `trench_source_samples.csv` / 同名工作表 | 全部已测 trench 的原始逐点边缘，包括未入选和不合格候选；主逐点表的 trench_source_id 可与本表的 source_trench_id 对照。 |
| `line_trench_pairs.csv` / 同名工作表 | line 的左右来源 trench ID、合格性、相邻性、选择状态及失败原因。 |
| `line_source_samples.csv` / 同名工作表 | line 流程搜索到的全部 trench 逐点边缘，包含 source_trench_id 与合格性；可与 per_sample_results 的左右来源 ID 和采样序号逐点核对。 |
...
| `machine_comparison_detailed.csv`、`V1_19_statistics_and_machine_comparison.xlsx` | 有参考表时的逐图机台对比和统计摘要；另查 unmatched 表定位匹配问题。 |
...
自检脚本 `self_check_V1_19.py` 不是主测量入口，它的参数为：

| 参数 | 默认值 | 含义 |
|---|---|---|
| `-h` / `--help` | 不启用 | 查看自检脚本帮助。 |
| `--trench-only` | 不启用 | 仅运行解析检查及 trench 全测后筛选专项回归；不能与 formats-only 同用。 |
| `--formats-only` | 不启用 | 仅运行解析检查及多格式图片集成测试；不能与 trench-only 同用，不能代替全部回归。 |
...
```bash
python self_check_V1_19.py --quick
python self_check_V1_19.py --output my_validation_report.json
python sem_cd_measure_200k_batch_V1_19.py --help
```

随包报告包含原有端到端/故障场景，以及新增模式判断、两种定位、端部与 PSD 排除、参数检查；这些是软件回归与随包/合成图验证，不替代真实 SEM 标准样验证。

本手册的覆盖范围以主程序 `build_parser()` 的全部选项为准：编写后逐项检查参数表是否包含每个公开参数，并使用实际解析器检查示例命令及相互冲突的参数。内部 `via_reference_nm` 等历史兼容字段并不是公开接口。本版使用独立 V1_19 包；原 V1_14 至 V1_18 的代码、文档、测试报告和 ZIP 与修改前提交 6c4ea0b 保持字节一致。包内附有同内容的 RUN_GUIDE.md，离线解压即可查阅全部参数。
...
`--pattern trench` 表示测暗沟槽；`--locator-mode bright-line` 表示沟槽之间的 line 整体亮。因此测 trench 时可以且通常应该使用 bright-line，不能把它误写成 `--pattern line`。`--pattern line` 也在原图中用同一套 trench 定位规则，再把相邻两条合格 trench 之间的间隙作为 line；模式名称始终描述原图中的分隔带。
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --locator-mode auto --pixel-size 1.3181 --trench-reference-nm 60 --output result_auto --no-auto-machine-comparison
```

已知 line 内有假暗条：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --locator-mode dark-line --pixel-size 1.3181 --trench-reference-nm 60 --output result_dark_line --no-auto-machine-comparison
```

已知 line 整体亮：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --locator-mode bright-line --locator-majority 0.7 --end-trim-fraction 0.05 --pixel-size 1.3181 --trench-reference-nm 60 --output result_bright_line --no-auto-machine-comparison
```

对照 V1_14 关闭自动 ROI 时的边缘配方（保留 bounded 搜索，最终 trench 筛选按 V1_19 执行）：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root input --pattern trench --locator-mode dark-line --no-auto-roi --end-trim-fraction 0 --pixel-size 1.3181 --trench-reference-nm 60 --output result_v114_compatible --no-auto-machine-comparison
```

若复现的是 V1_14 的 threshold-search=legacy 配方，还需同步加上 `--threshold-search legacy`。其余标定、ROI、采样、参考宽度和统计参数也须一致。V1_19 排除不合格 trench，参与汇总的对象可能变化，因此不承诺复现旧版纳入不合格对象的汇总值；同一来源 trench 的逐点算法保持不变。开启新区域裁剪会改变参与测量的像素/长度，因此不能要求结果与未裁剪版本数值一致。


## 12. line 修复与自动 pitch CD（2026-09-21）

### 12.1 继承 V1_17：由相邻 trench 搜索 line

V1_16 仍把原图反相，再单独寻找亮 line。line 中的暗条反相后变成亮条，可能把一条完整 line 拆开。V1_16 已处理部分情况，但当内部暗条较宽、两侧亮带较窄时仍会没有合格候选；同一原图中的 trench 却能正常测量。

V1_19 保持 V1_17 的以下 line 流程：

1. 在原图中使用现有 trench 流程完成背景排除、端部裁剪、ROI、dark-line/bright-line 定位，搜索 ROI 内全部合规 trench。来源搜索不受 max-number 提前截断。
2. 每个来源 trench 使用原有 V10/V13 边缘检测、跟踪、Viterbi/ERF、飞点及 continuity 处理，按原稳定性条件判断合格：非合成有效点比例达到 minimum-valid-fraction，统计点数至少为 max(2,ddof+1)。
3. 按 X 排序，仅两条物理相邻且均合格的 trench 构成 line。共享采样 Y 上，`line.left = left_trench.right`、`line.right = right_trench.left`。两边都有效、未被背景排除且没有交叉才有效。不会跨越不合格或漏检的物理 trench，也不会把外侧背景当作 line。
4. 形成全部合规相邻间隙后，按测量中心及左右邻居选择最多 max-number 条 line。CD/LER/LWR、旋转、分组、mixed 引擎分工及 PSD 沿用原统计方法。line 不单独重新拟合/移动共享边缘；可选边缘修正均在来源 trench 上完成。单行 PSD 提边缘同样使用共享方法。

`--line-reference-nm` 仍填写 line 宽度，兼容的 `--trench-reference-nm` 在 line 模式仍是 line 宽度的别名；`--space-reference-nm` 填物理 trench 宽度，不填则自动估计。例如 line=100nm、trench=60nm，使用 `--line-reference-nm 100 --space-reference-nm 60`。参数含义与 V1_16 保持兼容。

来源 trench 的左右阈值按原方向应用：line 左边沿用左 trench 的右阈值，line 右边沿用右 trench 的左阈值。`per_sample_results` 的 `left_trench_id/right_trench_id` 及两个共享坐标字段可与 `line_source_samples` 对照；`line_trench_pairs` 保存配对、选择及拒绝原因。合成补点保持标记，仍不计入合格率或 pitch。

### 12.2 pitch 定义、旋转和输出

一个 pitch = 一个完整 line + 紧邻的一个完整 trench。使用相邻同类结构的**左边缘到左边缘**定义周期：两个模式都采用物理 trench 的左边缘到相邻 trench 左边缘，即一个 trench + 一个 line。使用实际边缘，不直接相加两个独立平均 CD，也不使用名义参考节距作为测量值。
...
trench 和 line 模式都复用全部已测且合格的来源 trench。最终未被主测量选中的合格 trench 也可服务 pitch；两条 trench 已可构成一条 line 和一个完整 pitch，不要求第三条 trench。周期需满足相邻候选序号和观测周期检查：bright-line 的候选序号差为 1；dark-line 允许跨过一条假暗条（序号差最多 2）；距离须为观测周期的 0.60–1.40 倍。此检查使用自动建议的实际形态，避免手动模式把节距翻倍。不跨被漏检结构，不把两倍节距当作一个 pitch。默认要求同一周期至少 2 个共同有效点，且有效比例达到 minimum-valid-fraction。
...
```bash
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_dark_line --pattern line --pixel-size 1 --line-reference-nm 100 --space-reference-nm 60 --max-number 3 --output result_line_pitch --no-auto-machine-comparison
```

例：同图测 trench 并自动测相邻周期：

```bash
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_dark_line --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 3 --output result_trench_pitch --no-auto-machine-comparison
```

完整自检仍用 `python self_check_V1_19.py --output validation_report.json`；line/pitch 测试在 `check_line_pitch.py`，共享边缘测试在 `check_shared_lines.py`。覆盖逐点共享边验证、宽暗条、自动估宽、Viterbi/ERF、左右不同阈值、全部 trench 搜索、max-number=1/2/5、只有一条/两条 trench、不合格及缺失中间 trench、独立 PSD、倾斜周期、背景/合成点排除和 Excel 末列验证。随包样例及新增图均为合成图，尚无用户实测失败 SEM 图用于验证。


## 13. V1_18：trench 先全测、再筛选（2026-09-22）

V1_17 的 trench 在逐点测量前先选定几条候选，可能保留断续的沟槽，默认旧中心选择还可能选到假暗条；line 则先测全部来源 trench，只用相邻且合格的来源构成 line。V1_18 将 trench 的测量/筛选顺序调整为后者，V1_19 沿用该流程，最终输出仍是 trench 本身。

1. 保留原 ROI、背景、端部裁剪及 dark-line/bright-line 定位，搜索 ROI 内全部通过盆地和真实 trench 亮度判定的候选。内部搜索不受 max-number 限制，不用中心假暗条凑数。
2. 对每个候选调用原 `measure_single_space`，V10/V13 阈值、跟踪、Viterbi/ERF、飞点、continuity 和原始采样点不变。
3. 使用原 stable 条件筛选：非合成有效点数 / 非背景采样数达到 minimum-valid-fraction（默认 0.70），统计点数至少为 max(2,ddof+1)。合成补点不增加真实检测率，背景继续排除。
4. 仅在合格集合中按原中心/左右及显式 space-reference 间距先验选择，最多保留 max-number 条。不合格对象不进入主 CD/LER/LWR、标注和 PSD；数量不足时不会用它们补齐。
5. 独立单行 PSD 同样先全测再筛选，仍使用原 periodogram、分段和不平均策略。line 原流程不变。pitch 复用全部合格来源的同侧边缘，不跨过不合格或漏检的物理沟槽，按 max-number 个完整周期等权平均，只输出旋转结果，主表仍放最后一列。

`trench_selection` 保存合格性、检测比例、合成点数和入选原因；`trench_source_samples` 保留全部候选原始采样；主表逐点坐标与选中来源完全一致。定位候选表的 selected 现在表示最终入选，并附 source_measured/source_qualified。

只有一条合格 trench 时仍可测 CD，但没有完整 pitch；若全部 trench 均不合格，保留审计但该引擎主测量为空。一个引擎无结果时继续保留另一个引擎的有效部分。被判合格不代表所有采样点都连续，断点处理仍由原参数控制。搜索全部候选可能增加运行时间和审计文件体积。主汇总及 pitch 的可用对象集合扩大并经过质量筛选，入选对象改变时最终均值也可能变化；这不代表边缘算法或统计公式改变。

```bash
python sem_cd_measure_200k_batch_V1_19.py --root examples/input_dark_line --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 3 --output results_trench_V1_19 --no-auto-machine-comparison
python self_check_V1_19.py --trench-only --output trench_validation.json
python self_check_V1_19.py --output validation_report.json
```

专项测试见 `check_trench_selection.py`，包括 max-number=1/2/5 仍全量搜索、bright-line 近邻选择、假中心排除、不合格中心替换、合格数量不足/为零、单沟槽、左右不等阈值及 Viterbi/ERF 下 line/trench 来源逐点一致、独立 PSD 筛选。旧版本的边缘公式通过源码和逐点数值对照校验；旧的低支持率汇总用例改为验证筛除与原始坐标保留。


## 14. V1_19：多格式图片输入（2026-09-23）

输入目录支持 `.tif`、`.tiff`、`.png`、`.jpg`、`.jpeg`、`.bmp`、`.webp`，扩展名不区分大小写，可在 root 及其子目录混合放置。`--root` 仍为目录，不是单个文件；不新增必填参数或依赖。所有格式共用原先的 ROI、双边缘、trench/line、pitch 与 PSD 流程。

- 支持中文和带空格的路径。当前输出目录及 V1_14 至 V1_19 等可识别历史结果目录继续排除。
- 8 位灰度保持原强度；彩色/带透明通道图片按原规则转灰度，透明通道不参与测量。16 位 TIFF/PNG 保留位深解码，再按 V1_18 原有的预扫描/边缘归一化规则处理。低动态范围图请核查 ROI 和标注。
- 多页 TIFF 仅读取第一页；不逐页批量测量，不从 TIFF 的 DPI/其他元数据自动设置 pixel-size。像素尺寸仍由 `--pixel-size` 指定。
- JPEG 是有损格式，压缩可能改变边缘、CD 和粗糙度；同一图片转成 JPEG 后不保证数值与无损原图相同。
- 无法解码的支持格式文件记录在 `processing_errors.csv` 和汇总 ERROR 行，默认继续处理可读文件。非支持扩展名不会扫描。预扫描错误继续遵守第 9 节所述行为。

输出图片统一为 PNG，保留输入的完整文件名（包括扩展名），并镜像相对目录：

| 输入 | ROI/标注图文件名 | debug 文件名（示例） | PSD 文件名 |
|---|---|---|---|
| `sample.tif` | `sample.tif.png` | `sample.tif_V10_rotated.png` | `sample.tif.psd.png` |
| `sample.jpg` | `sample.jpg.png` | `sample.jpg_V10_rotated.png` | `sample.jpg.psd.png` |
| `sample.png` | `sample.png.png` | `sample.png_V10_rotated.png` | `sample.png.psd.png` |

V1_19 的输出接口变化：`settings.json` 的计数字段改为 `input_image_count` 和 `processed_image_count`，增加 `supported_image_suffixes`；`machine_unmatched_png.csv` 改为 `machine_unmatched_images.csv`，其 Excel 未匹配类型为 `image`。计数仍包含读取失败的 ERROR 记录，与旧版语义一致。CSV/Excel 的 image 和 image_key 保留真实文件名/相对路径。机台对照保留去扩展名匹配规则；例如 sample.tif 与 sample.png 同时存在时，不任意选择其中一张，而是将同名歧义及原因写入未匹配表。

`check_image_formats.py` 使用实际编码文件检查 TIFF/PNG/JPEG/BMP/WebP、大小写、中文子目录、16 位、RGB/RGBA、多页第一页、混合批处理、导出防覆盖、机台匹配和损坏文件；完整自检保留 V1_18 原有全部回归。`audits/v1_19_release_evidence.json` 记录旧版本文件、PNG 数值一致性与独立解压验证。所有测试使用合成图。
