...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V1_19: background-aware dual-edge measurement and unaveraged PSD.
...
SCRIPT_VERSION = "V1_19"
DEFAULT_ROOT = regions.DEFAULT_ROOT
DEFAULT_OUTPUT_NAME = "CD_measure_output_200K_V1_19"
EPS = 1e-12
PUBLIC_GROUP_SIZE = 4
V113_METHOD_CODE = "V1_19_DualCoordinates_V13CD_V10LER_V13LWR"
...
OUTPUT_SCHEMA = "metadata_first_methods_raw_psd_v119_qualified_trenches"
PATCH_VERSION = "V1_19"
RESULT_COLUMNS = [
...
def build_v10_params(
    ...
    """Create one V10 parameter set while allowing arbitrary image dimensions."""
...
def generate_coordinate_statistics(
    ...
    write_csv(summary, output_dir / "V1_19_statistics_summary.csv")
    write_csv(pd.DataFrame(plot_rows), output_dir / "V1_19_plot_index.csv")
    write_workbook(
        output_dir / "V1_19_statistics_and_machine_comparison.xlsx",
        {
...
def build_machine_comparison(
    ...
    unmatched_images = calc[~calc["match_key"].isin(matched_keys)].copy()
    if not duplicated.empty:
        duplicated = duplicated.assign(
            unmatched_reason="duplicate_basename_in_recursive_tree"
        )
        unmatched_images = pd.concat(
            [unmatched_images, duplicated], ignore_index=True
    ...
    write_csv(unmatched_images, output_dir / "machine_unmatched_images.csv")
    ...
    return (
        ...
        pd.concat(
            [
                ...
                unmatched_images.assign(unmatched_type="image"),
            ],
            ignore_index=True,
            sort=False,
        ),
...
def save_internal_plots(image_df, output_dir, dpi):
    ...
    for col, mode in enumerate(MODES):
        for row, metric in enumerate(METRICS):
            ...
            ax.legend()
        axes[-1, col].set_xlabel("Processed image index")
    fig.suptitle("V1_19: rotated first, unrotated second")
    fig.tight_layout()
    fig.savefig(
...
def process_trench(
    ...
    if not parsed_images and not discovery_errors:
        raise RuntimeError(
            "没有找到可处理的图片（TIF/TIFF、PNG、JPG/JPEG、BMP、WebP）；"
            "程序递归扫描 root 并排除输出目录"
        )
    print(
    ...
    trench_selections, trench_sources = [], []
    roi_rows = []
    locator_rows = []
    locator_candidates = []
    ...
    for number, parsed in enumerate(parsed_images, start=1):
        ...
        try:
            gray = regions.read_gray_image(parsed.path)
            search_reference_nm = (
                parsed.design_space_nm
                if args.pattern == "line"
            ...
            for engine, measurement in (("V10", m10), ("V13", m13)):
                if measurement is not None:
                    ...
                    trench_selections.extend(
                        {"image_key": key, "engine": engine, **row}
                        for row in payload.get("trench_selection", [])
                    )
                    trench_sources.extend(
                        {"image_key": key, "engine": engine, **row}
                        for row in payload.get("trench_source_samples", [])
                    )
                    line_pairs.extend(
                        {"image_key": key, "engine": engine, **row}
            ...
            for engine, measurement, params, samples, module in (
                    ...
                    actions.append(
                        (
                            ...
                            lambda: module.save_annotated_image(
                                ...
                                output_dir
                                / f"annotated_未旋转_{engine}"
                                / regions.output_image_path(key),
                                params,
                            ),
                        )
                    )
                if args.save_debug_masks:
                    target = (
                        output_dir
                        / "debug"
                        / regions.output_image_path(key, f"_{engine}_rotated.png")
                    )
                    actions.append(
                        (
                            "diagnostic",
            ...
            if region is not None:
                try:
                    regions.save_region_overlay(
                        gray,
                        region,
                        output_dir / "ROI" / regions.output_image_path(key),
                    )
                except Exception as exc:
                    errors.append(
                        {
            ...
            try:
                ...
                regions.save_region_overlay(
                    gray, empty, output_dir / "ROI" / regions.output_image_path(key)
                )
    ...
    frames = {
        ...
        "trench_selection": dataframe_or_empty(
            trench_selections,
            [
                "image_key",
                "engine",
        ...
        "trench_source_samples": dataframe_or_empty(
            trench_sources,
            [
                "image_key",
                "engine",
    ...
    outputs = {
        "excel": output_dir / "CD_measurement_200K_V1_19_results.xlsx",
    ...
    settings = {
        ...
        "supported_image_suffixes": list(regions.SUPPORTED_IMAGE_SUFFIXES),
        "input_image_count": len(parsed_images) + len(discovery_errors),
        ...
        "trench_policy": "measure all eligible physical trenches, filter by original stability, then select max-number; no unqualified quota filling",
    ...
    print(
        f"\n处理完成：{len(image_rows)} 张图片，{len(object_df)} 个结构结果，{len(error_df)} 条错误。"
    )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "V1_19：同时输出旋转和未旋转结果；先旋转 mixed/V13/V10，再未旋转 mixed/V13/V10；"
            "V13计算CD/LWR，V10计算LER；递归处理 TIF/TIFF、PNG、JPG/JPEG、BMP、WebP（扩展名大小写不限），"
    ...
    parser.add_argument(
        ...
        choices=("periodogram",),
        default="periodogram",
        help="V1_19 PSD 不平均：逐结构、逐连续段 periodogram",
    )
    parser.add_argument(
