#!/usr/bin/env python3
"""Build the self-contained V1_17 release archive; exclude caches and outputs."""
...
def main():
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Explicitly replace the V1_17 archive only",
    )
    ...
    if not (PACKAGE / "validation_report.json").is_file():
        raise SystemExit(
            "Run self_check_V1_17.py --output validation_report.json before packaging"
        )
