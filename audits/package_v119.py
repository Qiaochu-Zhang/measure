#!/usr/bin/env python3
"""Build the self-contained V1_19 release archive; exclude caches and outputs."""
...
def main():
    ...
    parser.add_argument(
        "--replace",
        action="store_true",
        help="Explicitly replace the V1_19 archive only",
    )
    ...
    if not (PACKAGE / "validation_report.json").is_file():
        raise SystemExit(
            "Run self_check_V1_19.py --output validation_report.json before packaging"
        )
    ...
    manifest.write_text("\n".join(lines) + "\n")
    # Publish only the complete, verified archive.
    with TemporaryDirectory(prefix=".v119-package-", dir=ROOT) as directory:
        ...
        with zipfile.ZipFile(temporary) as result:
            ...
            print(
                f"Created {archive.name}: {len(result.infolist())} files, {temporary.stat().st_size} bytes"
            )
