...
def analytical_tests():
    ...
    require(
        app.SCRIPT_VERSION == "V1_17" and app.PATCH_VERSION == "V1_17",
        "independent V1_17 version",
    )
    with tempfile.TemporaryDirectory(prefix="v117-version-check-") as directory:
        root = Path(directory)
        gray = app.regions.read_gray_png(BASE / "examples/input_trench/five/test.png")
        app.regions.unicode_imwrite(root / "input.png", gray)
        for version in ("V1_14", "V1_15", "V1_16", "V1_17"):
        ...
        require(
            config.resolved_output_dir().name == "CD_measure_output_200K_V1_17",
            "V1_17 default output",
        )
    ...
    report["v117_version_isolation"] = {
        "status": "PASS",
        "excluded_output_versions": ["V1_14", "V1_15", "V1_16", "V1_17"],
    }
...
def integration_tests(temp):
    ...
    def execute(name, root=None, extra=(), pattern="trench", reference=60):
        ...
        book = load_workbook(
            output / "CD_measurement_200K_V1_17_results.xlsx", read_only=True
        )
        require(
...
if __name__ == "__main__":
    ...
    if not args.quick:
        with tempfile.TemporaryDirectory(prefix="measure-v117-check-") as temp:
    ...
    print("V1_17 SELF-CHECK PASSED", flush=True)
