...
适用版本：`V1_17`（2026-09-21）。主程序：`sem_cd_measure_200k_batch_V1_17.py`，基于 V1_16（c0ae220），将 line 改为相邻合格 trench 之间的共享边缘测量，保留自动 pitch 测量，按本次 V1_17 发布的实际参数解析、校验和执行代码编写。本文覆盖主程序全部 **86 个参数选项（包括帮助选项，`-h` 和 `--help` 为同一个选项）**，另列自检脚本参数。没有在命令行开放的内部常量，不应当作可传入参数。

- [完整代码 ZIP](sem_cd_measure_200k_batch_V1_17_complete_package.zip)
- [版本概述](V1_17_RELEASE_NOTES.md)
- [包内 README](sem_cd_measure_200k_batch_V1_17_complete_package/README.md)
- [主程序源码](sem_cd_measure_200k_batch_V1_17_complete_package/sem_cd_measure_200k_batch_V1_17.py)

## 1. 先了解默认行为

程序用于 PNG 图像中近竖直的暗沟槽 `trench` 或亮线 `line` 的 CD、LER、LWR 测量。一次运行的整个输入目录只能使用一种图案类型；不同类型请分开运行。本版不支持 via，也不自动逐图判断 trench/line。V1_17 可单独解压运行，输出版本标识、Excel 及默认目录均为 V1_17；递归扫描仍排除 V1_14/V1_15/V1_16/V1_17 的历史结果。建议不同版本结果放在输入目录之外。

默认会：

1. 递归读取输入目录及子目录的 PNG，不按文件名或 `200K` 字样筛选；当前输出目录和可识别的历史输出子目录会被排除。
2. 筛除背景，按估计平均 trench 长度从 Y 两端各去掉 5%，自动确定未手动指定的 ROI；默认每图最多选 3 条结构；line 先搜索 ROI 内全部合规 trench，再选择最多 max-number 条相邻间隙。根据整个有效区域的 X 灰度周期逐图选择 dark-line 或 bright-line 定位器。
...
解压完整 ZIP，进入包含主程序和 `requirements_V1_17.txt` 的目录。建议 Python 3.12；代码使用 Python 3.10+ 语法，实际测试版本见包内 `validation_report.json`。
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root examples/input_trench --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 4 --output demo_output --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root "D:/SEM/input_trench" --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output "D:/SEM/results_trench_01" --no-auto-machine-comparison
```

亮线：

```bash
python sem_cd_measure_200k_batch_V1_17.py --root "D:/SEM/input_line" --pattern line --pixel-size 1.3181 --line-reference-nm 60 --output "D:/SEM/results_line_01" --no-auto-machine-comparison
```

Linux/macOS 把路径替换为 `/path/to/images` 等实际路径。相对路径基于终端的当前工作目录；含空格的路径加引号。Windows 也可使用 `./RUN_V1_17.ps1`，Linux/macOS 可用 `./run_V1_17.sh` 替代 `python sem_cd_measure_200k_batch_V1_17.py`，参数相同。PowerShell 包装脚本未在本次 Linux 环境执行验证。
...
| `--output` | 路径；自动 | 默认 `root/CD_measure_output_200K_V1_17`。目录必须不存在或为空。不要直接指定到包含输入图的目录。 |
...
| `--max-number` | 整数；`3` | 每张图每个引擎最多选取的结构数量，≥1；同时为 pitch 求平均的目标周期数。N 个完整 pitch 需要 N+1 条物理 trench 的边缘；line 会先搜索全部合规 trench，不提前受此数量截断。不是图片数量，也不是采样点数。 |
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output results_basic --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --no-auto-roi --center-x 300 --center-y 250 --meas-area-width 400 --meas-area-height 300 --extend-length 240 --output results_manual --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern line --pixel-size 1.3181 --line-reference-nm 60 --max-number 1 --min-number 1 --group-size 1 --output results_single --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source single-row --psd-gap-mode interpolate --psd-max-gap 2 --output results_psd_interpolate --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source measurement --output results_psd_reuse --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --viterbi 1 --viterbi-weight 5 --viterbi-max-jump 3 --erf-fit 1 --erf-window 8 --save-debug-masks --output results_path_erf --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --skip-psd --skip-statistics-plots --no-annotated-images --compact-sample-output --output results_fast --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --pattern trench --max-number 4 --group-size 8 --check-parameters
```
...
先打开 `CD_measurement_200K_V1_17_results.xlsx` 的 `measurement_summary`，再核查 `ROI/` 和双引擎标注图。
...
| `machine_comparison_detailed.csv`、`V1_17_statistics_and_machine_comparison.xlsx` | 有参考表时的逐图机台对比和统计摘要；另查 unmatched 表定位匹配问题。 |
...
自检脚本 `self_check_V1_17.py` 不是主测量入口，它的参数为：
...
python sem_cd_measure_200k_batch_V1_17.py --help
```

随包报告包含原有端到端/故障场景，以及新增模式判断、两种定位、端部与 PSD 排除、参数检查；这些是软件回归与随包/合成图验证，不替代真实 SEM 标准样验证。

本手册的覆盖范围以主程序 `build_parser()` 的全部选项为准：编写后逐项检查参数表是否包含每个公开参数，并使用实际解析器检查示例命令及相互冲突的参数。内部 `via_reference_nm` 等历史兼容字段并不是公开接口。本版使用独立 V1_17 包；原 V1_15、V1_16 的代码、文档、测试报告和 ZIP 与修改前提交 c0ae220 保持字节一致（V1_15 已在该提交恢复为 41e3294）。包内附有同内容的 RUN_GUIDE.md，离线解压即可查阅全部参数。
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --locator-mode auto --pixel-size 1.3181 --trench-reference-nm 60 --output result_auto --no-auto-machine-comparison
```

已知 line 内有假暗条：

```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --locator-mode dark-line --pixel-size 1.3181 --trench-reference-nm 60 --output result_dark_line --no-auto-machine-comparison
```

已知 line 整体亮：

```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --locator-mode bright-line --locator-majority 0.7 --end-trim-fraction 0.05 --pixel-size 1.3181 --trench-reference-nm 60 --output result_bright_line --no-auto-machine-comparison
```

复现 V1_14 关闭自动 ROI 时的行为（仍使用 V1_14 默认 bounded 边缘搜索）：

```bash
python sem_cd_measure_200k_batch_V1_17.py --root input --pattern trench --locator-mode dark-line --no-auto-roi --end-trim-fraction 0 --pixel-size 1.3181 --trench-reference-nm 60 --output result_v114_compatible --no-auto-machine-comparison
```
...
trench 模式保持原有额外测一个相邻 trench 的策略，额外结构仅服务 pitch。line 模式直接复用全部已测来源 trench；两条 trench 已可构成一条 line 和一个完整 pitch，不要求第三条 trench。周期需满足相邻候选序号和观测周期检查：bright-line 的候选序号差为 1；dark-line 允许跨过一条假暗条（序号差最多 2）；距离须为观测周期的 0.60–1.40 倍。此检查使用自动建议的实际形态，避免手动模式把节距翻倍。不跨被漏检结构，不把两倍节距当作一个 pitch。默认要求同一周期至少 2 个共同有效点，且有效比例达到 minimum-valid-fraction。
...
```bash
python sem_cd_measure_200k_batch_V1_17.py --root examples/input_dark_line --pattern line --pixel-size 1 --line-reference-nm 100 --space-reference-nm 60 --max-number 3 --output result_line_pitch --no-auto-machine-comparison
```

例：同图测 trench 并自动测相邻周期：

```bash
python sem_cd_measure_200k_batch_V1_17.py --root examples/input_dark_line --pattern trench --pixel-size 1 --trench-reference-nm 60 --max-number 3 --output result_trench_pitch --no-auto-machine-comparison
```

完整自检仍用 `python self_check_V1_17.py --output validation_report.json`；line/pitch 测试在 `check_line_pitch.py`，共享边缘测试在 `check_shared_lines.py`。覆盖逐点共享边验证、宽暗条、自动估宽、Viterbi/ERF、左右不同阈值、全部 trench 搜索、max-number=1/2/5、只有一条/两条 trench、不合格及缺失中间 trench、独立 PSD、倾斜周期、背景/合成点排除和 Excel 末列验证。随包样例及新增图均为合成图，尚无用户实测失败 SEM 图用于验证。
