...
适用版本：`sem_cd_measure_200k_batch_V1_14.py`，按仓库提交 `e2c4607` 的实际参数解析、校验和执行代码编写。本文覆盖主程序全部 **83 个参数选项（包括帮助选项，`-h` 和 `--help` 为同一个选项）**，另列自检脚本参数。没有在命令行开放的内部常量，不应当作可传入参数。

- [完整代码 ZIP](sem_cd_measure_200k_batch_V1_14_complete_package.zip)
- [版本概述](V1_14_RELEASE_NOTES.md)
- [包内 README](sem_cd_measure_200k_batch_V1_14_complete_package/README.md)
- [主程序源码](sem_cd_measure_200k_batch_V1_14_complete_package/sem_cd_measure_200k_batch_V1_14.py)
...
2. 筛除背景，自动确定未手动指定的 ROI（测量区域）；每图最多选 3 条结构。
...
解压完整 ZIP，进入包含主程序和 `requirements_V1_14.txt` 的目录。建议 Python 3.12；代码使用 Python 3.10+ 语法，实际测试版本见包内 `validation_report.json`。
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root "D:/SEM/input_trench" --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output "D:/SEM/results_trench_01" --no-auto-machine-comparison
```

亮线：

```bash
python sem_cd_measure_200k_batch_V1_14.py --root "D:/SEM/input_line" --pattern line --pixel-size 1.3181 --line-reference-nm 60 --output "D:/SEM/results_line_01" --no-auto-machine-comparison
```

Linux/macOS 把路径替换为 `/path/to/images` 等实际路径。相对路径基于终端的当前工作目录；含空格的路径加引号。Windows 也可使用 `./RUN_V1_14.ps1`，Linux/macOS 可用 `./run_V1_14.sh` 替代 `python sem_cd_measure_200k_batch_V1_14.py`，参数相同。PowerShell 包装脚本未在本次 Linux 环境执行验证。

输入建议：裁去文字、标尺等非测量内容，保留原始像素和正确标定；不要将 trench、line 和历史输出图混放。彩色图会转灰度，8bit 灰度保持原强度；非 8bit 图像会归一化，需要额外检查低动态范围图像。
...
| `--output` | 路径；自动 | 默认 `root/CD_measure_output_200K_V1_14`。目录必须不存在或为空。不要直接指定到包含输入图的目录。 |
...
| 参数 | 类型 / 默认值 | 含义与注意事项 |
|---|---|---|
| `--no-auto-roi` | 开关；默认不启用 | 关闭自动背景排除及自动 ROI 调整，改用手动值或图像中心默认值。并不是禁止边缘检测。 |
...
| `--center-x` | 浮点数；自动 | 原图坐标中的测量中心 X，向右增大。自动 ROI 开启且未指定时用保留区域包围框中心；关闭自动 ROI 时用 `(图宽−1)/2`。必须在图内。 |
| `--center-y` | 浮点数；自动 | 原图坐标中的测量中心 Y，向下增大。自动 ROI 开启且未指定时用保留区域包围框中心；关闭时用 `(图高−1)/2`。必须在图内。 |
| `--meas-area-width` | 整数；自动 | 粗定位 ROI 宽度，px，必须 >0 且不超过图宽。自动 ROI 时按保留区域范围及中心到图边的距离调整；关闭自动 ROI 时默认 `min(512,图宽)`。不是最终测出的线宽。 |
| `--meas-area-height` | 整数；自动 | 粗定位 ROI 高度，px，必须 >0 且不超过图高。自动 ROI 时按保留区域范围调整；关闭时默认 `min(512,图高)`。不是最终沿线采样长度。 |
| `--extend-length` | 整数；自动 | 沿 Y 方向的测量长度，必须在 2 到图像高度之间。自动 ROI 未显式设置时为 `max(2,min(360,保留区域包围框高度−average-range))`；关闭自动 ROI 时为 `min(360,图高)`。影响主测量和 PSD 测量范围。 |

显式填写的中心、区域尺寸和长度优先于自动值，但**手动 ROI 不会关闭背景掩码**。若想完全手动测量，需同时加入 `--no-auto-roi`。坐标从左上角 `(0,0)` 开始，始终属于原图；程序不会先裁剪再重新编号。
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --output results_basic --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --no-auto-roi --center-x 300 --center-y 250 --meas-area-width 400 --meas-area-height 300 --extend-length 240 --output results_manual --no-auto-machine-comparison
```

示例中心和尺寸必须适合你的原图。只填写 center-x/center-y 不等于关闭背景排除。

### 5.4 只测一条线，并输出未分组主 LWR

```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern line --pixel-size 1.3181 --line-reference-nm 60 --max-number 1 --min-number 1 --group-size 1 --output results_single --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source single-row --psd-gap-mode interpolate --psd-max-gap 2 --output results_psd_interpolate --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --psd-edge-source measurement --output results_psd_reuse --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --viterbi 1 --viterbi-weight 5 --viterbi-max-jump 3 --erf-fit 1 --erf-window 8 --save-debug-masks --output results_path_erf --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --root input --pattern trench --pixel-size 1.3181 --trench-reference-nm 60 --skip-psd --skip-statistics-plots --no-annotated-images --compact-sample-output --output results_fast --no-auto-machine-comparison
```
...
```bash
python sem_cd_measure_200k_batch_V1_14.py --pattern trench --max-number 4 --group-size 8 --check-parameters
```
...
6. 手动中心/尺寸优先，但不会关闭自动背景掩码；no-auto-roi 才关闭自动区域筛选。
...
先打开 `CD_measurement_200K_V1_14_results.xlsx` 的 `measurement_summary`，再核查 `ROI/` 和双引擎标注图。
...
| `per_sample_results` | 主测量每个采样位置的边缘、有效性、失败原因及补点标记。 |
| `ROI/`、`roi_summary.csv` | 背景保留区域图和原图坐标范围；绿色框是包围框，暗化部分为排除背景。 |
...
| `machine_comparison_detailed.csv`、`V1_14_statistics_and_machine_comparison.xlsx` | 有参考表时的逐图机台对比和统计摘要；另查 unmatched 表定位匹配问题。 |
...
自检脚本 `self_check_V1_14.py` 不是主测量入口，它的参数为：
...
```bash
python self_check_V1_14.py --quick
python self_check_V1_14.py --output my_validation_report.json
python sem_cd_measure_200k_batch_V1_14.py --help
```

随包报告包含 17 项端到端/故障场景及数学检查；这些是软件回归与随包/合成图验证，不替代真实 SEM 标准样验证。

本手册的覆盖范围以主程序 `build_parser()` 的全部选项为准：编写后逐项检查参数表是否包含每个公开参数，并使用实际解析器检查示例命令及相互冲突的参数。内部 `via_reference_nm` 等历史兼容字段并不是公开接口。本次只新增运行文档和索引链接，不修改测量算法、原发布 ZIP 或其校验清单。
