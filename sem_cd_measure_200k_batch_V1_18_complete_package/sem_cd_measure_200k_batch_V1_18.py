...
#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""V1_18: background-aware dual-edge measurement and unaveraged PSD.
...
SCRIPT_VERSION = "V1_18"
DEFAULT_ROOT = regions.DEFAULT_ROOT
DEFAULT_OUTPUT_NAME = "CD_measure_output_200K_V1_18"
EPS = 1e-12
PUBLIC_GROUP_SIZE = 4
V113_METHOD_CODE = "V1_18_DualCoordinates_V13CD_V10LER_V13LWR"
...
OUTPUT_SCHEMA = "metadata_first_methods_raw_psd_v118_qualified_trenches"
PATCH_VERSION = "V1_18"
RESULT_COLUMNS = [
...
def generate_coordinate_statistics(
    ...
    write_csv(summary, output_dir / "V1_18_statistics_summary.csv")
    write_csv(pd.DataFrame(plot_rows), output_dir / "V1_18_plot_index.csv")
    write_workbook(
        output_dir / "V1_18_statistics_and_machine_comparison.xlsx",
        {
...
def save_internal_plots(image_df, output_dir, dpi):
    ...
    fig.suptitle("V1_18: rotated first, unrotated second")
    fig.tight_layout()
    fig.savefig(
...
def process_trench(
    ...
    outputs = {
        "excel": output_dir / "CD_measurement_200K_V1_18_results.xlsx",
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "V1_18：同时输出旋转和未旋转结果；先旋转 mixed/V13/V10，再未旋转 mixed/V13/V10；"
    ...
    parser.add_argument(
        ...
        choices=("periodogram",),
        default="periodogram",
        help="V1_18 PSD 不平均：逐结构、逐连续段 periodogram",
    )
    parser.add_argument(
