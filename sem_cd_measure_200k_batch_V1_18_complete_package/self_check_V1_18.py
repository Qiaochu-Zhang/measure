...
def analytical_tests():
    ...
    require(
        app.SCRIPT_VERSION == "V1_18" and app.PATCH_VERSION == "V1_18",
        "independent V1_18 version",
    )
    with tempfile.TemporaryDirectory(prefix="v118-version-check-") as directory:
        root = Path(directory)
        gray = app.regions.read_gray_png(BASE / "examples/input_trench/five/test.png")
        app.regions.unicode_imwrite(root / "input.png", gray)
        for version in ("V1_14", "V1_15", "V1_16", "V1_17", "V1_18"):
        ...
        require(
            config.resolved_output_dir().name == "CD_measure_output_200K_V1_18",
            "V1_18 default output",
        )
    ...
    report["v118_version_isolation"] = {
        "status": "PASS",
        "excluded_output_versions": ["V1_14", "V1_15", "V1_16", "V1_17", "V1_18"],
    }
...
def integration_tests(temp, trench_only=False):
    cases, cached = {}, {}
    ...
    def execute(name, root=None, extra=(), pattern="trench", reference=60):
        ...
        book = load_workbook(
            output / "CD_measurement_200K_V1_18_results.xlsx", read_only=True
        )
        require(
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--trench-only",
        action="store_true",
        help="Run analytical and V1_18 trench selection tests only",
    )
    ...
    if not args.quick:
        with tempfile.TemporaryDirectory(prefix="measure-v118-check-") as temp:
            result["integration"] = integration_tests(
                Path(temp), trench_only=args.trench_only
            )
    if args.output:
    ...
    print("V1_18 SELF-CHECK PASSED", flush=True)
